cc.Class({
        ...
        for(let i = 1; i < this.node.children.length;i++){
            ...
            this.node.children[i].on("touchmove",(event)=>{
                ...
                if(!this.addItemOnMeadow){
                    let x_pos = ((this.node.x + this.node.children[i].x) - (this.node.parent.children[window.m_i].x)) / this.node.parent.children[window.m_i].scaleX;
                    let y_pos = ((this.node.y + this.node.children[i].y) - (this.node.parent.children[window.m_i].y)) / this.node.parent.children[window.m_i].scaleY;
                    let name_object = this.node.children[i]._name;
                    this.node.parent.children[window.m_i]._components[1].spawnNewObject({
                            name:name_object,coord:{x:x_pos,y:y_pos},ID:10,shop:true,already_fed:false,time_start:0,square_data:[]
                        });
    ...
    spawn_yes_and_no(i,x,y){
        ...
        this.node.children[this.node.children.length - 2].on("touchend",(event)=>{
            ...
            this.node.parent._components[2].addMoney(-this.node.children[i]._components[1].import_data.price);
